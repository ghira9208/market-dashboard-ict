...
share every underlying module (detectors.py, theme.py, data.py, recommender.py,
...
with st.sidebar:
    ...
    if st.button("🔍 Scan watchlist", key="signals_scan_btn", width="stretch",
                 help="Scans the major coins for each one's own single best currently-active setup on 4h, "
                      "with 1W context weighted in (see recommender.py's own TF_PAIRS[0]) — see which coin "
                      "has the strongest read right now, not just whatever's already charted."):
        ...
        _scan_specs += [(get_yf_ohlcv, (t, _scan_context_conf["period"], _scan_context_conf["fetch_interval"],
                                         _prefetch_data_source), {}) for t in SCAN_TICKERS]
...
with main_col:
    ...
    def _render_layer_controls():
        ...
        with _tab_settings, st.container(key="_panel_settings"):
            ...
            _use_news_blackout = st.checkbox(
                ...
                help="Drops any past trade the win-rate/backtest numbers above would otherwise count if it "
                     ...
                     "currencies (e.g. USD news for BTC-USD) — real trades wouldn't take that entry either, "
        ...
        with _tab_strategy, st.container(key="_panel_strategy"):
            ...
            # shared with Markets' own Strategy tab — same backtest_results.db
            # underneath regardless of which of the two called it. Footprint
            # used to be a third sub-tab here too — moved to live directly
            # under the main chart instead (see the very end of this
            # fragment), so it can follow whatever ticker/timeframe you're
            # already looking at instead of asking you to re-pick both.
            # Markets never had it either way — Yahoo-only tickers (all
            # Markets ever shows) can't do this regardless.
            st.caption(
        ...
        try:
            ...
            if show_volume_profile and not _vp_df.empty:
                ...
                if _vp is not None:
                    ...
                    # Starts at the anchor's own first bar when anchored, not
                    # axis_secs[0] (the main chart's own earliest loaded bar)
                    # — a visual cue that this profile only reflects data
                    # from here forward, not the whole chart, matching what
    ...
    def _render_replay_chart(rdf, r_ticker, r_tf_label, r_data_source, r_n_total):
        ...
        # (offline replay of a real ticker at consecutive bars) that every
    ...
    # Footprint used to live in its own Strategy → Footprint sub-tab with
    # its own ticker/candle-size pickers — moved directly under the main
    # chart per direct request: it now follows whatever ticker/timeframe
    # is already on screen instead of asking for both again (still
    # overridable by hand inside — see render_footprint_tab's own
    # docstring on the sync behavior). Read fresh from session_state
    # rather than closing over _render_chart's own tf_label — that's a
    # separate fragment's own local, out of scope here. Collapsed by
    # default, and still gated behind its own "Load footprint" button
    # inside — real Binance trade data, can take several seconds,
    # deliberately never auto-fetches just because the ticker/TF changed.
    # Kept OUTSIDE _render_chart's own auto-ticking fragment on purpose:
    # _fetch_klines/_fetch_agg_trades are ttl-cached (20s/60s) but
    # _build_footprint_payload and the st.html() render below are NOT —
    # sitting inside a fragment that ticks every 1s on the main panel
    # would re-run both every tick regardless of whether the underlying
    # trades actually changed.
    with st.expander("📊 Footprint chart", expanded=False):
        _main_tf_now = st.session_state.get(TF_KEY_BY_CHART["main"], DEFAULT_TF_BY_CHART["main"])
        footprint.render_footprint_tab(TICKER_INFO, sync_ticker=ticker, sync_interval=_main_tf_now)
